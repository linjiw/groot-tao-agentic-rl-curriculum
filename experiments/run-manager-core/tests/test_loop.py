...
"""RunManager loop tests — the full Phase-2 test_smoke_driver.py suite
(45 tests) migrated onto the engine-agnostic core:

- SmokeDriver -> RunManager + LoopConfig (arm strings become flag
  bundles: control_config()/scripted_config());
- the test-local fake家族 -> adapters.mock formal protocol mocks;
- knob_registry.load_registry() SONIC default -> injected
  MOCK_REGISTRY_SPEC (same values for the knobs exercised);
- heldout manifest wiring -> heldout_hook + eval_extra_overrides pin
  (the v4 structural guard now lives in LoopConfig.__post_init__);
- purge -> injected purge_fn (adapters.mock.simple_purge — the
  engine-neutral half of the old purge; docker fallback stays SONIC-side);
- monkeypatch target for the disk gate: core.loop.shutil.
"""
...
from core.loop import DiskSpaceError, LoopConfig, RunManager, \
    control_config, scripted_config
from core.registry import ConfigDriftError, KnobRegistry, RunState
...
def make_registry():
    return KnobRegistry(copy.deepcopy(MOCK_REGISTRY_SPEC))


def make_manager(policy, adapter, config=None, **cfg_kw):
    return RunManager(policy, adapter, make_registry(),
                      config or LoopConfig(**cfg_kw))


def run_driver(script, arm="manager", segments=None, **policy_kw):
    fake = FakeAdapter(script)
    cfg = control_config() if arm == "control" else LoopConfig(arm=arm)
    driver = make_manager(TrainSideBandPolicy(**policy_kw), fake, cfg)
...
def test_hard_range_ceiling_stops_loosening():
    ...
    driver = make_manager(TrainSideBandPolicy(len_low=20, sustain=2),
                          FakeAdapter([(10, 1.0)] * 8))
...
def test_no_reward_baseline_refuses_change():
    ...
    driver = make_manager(TrainSideBandPolicy(len_low=20, sustain=2), fake)
    s = driver.run(4)
    assert s["decisions_applied"] == 0
...
def test_eval_runs_every_segment_both_arms():
    ...
    # eval records carry the loop's cumulative iteration counter
...
def test_registry_rejects_set_while_driver_watch_armed():
    """The validator ITSELF (not just the loop's gate) rejects a new 'set'
...
def test_decision_without_effect_check_scores_plain_survived():
    ...
    driver = make_manager(NoCheckPolicy(len_low=20, sustain=2), fake)
    driver.run(5)
    applied = next(e for e in driver.journal if e.get("applied"))
...
def test_config_verify_refuses_on_drift():
    """The v2 defect shape: belief says 0.35 but the run really launched at
    strict 0.2 -> refuse."""
    fake = ConfigFakeAdapter([(50, 1.0)] * 2, [_cfg_yaml(0.2)] * 2)
    driver = make_manager(
        TrainSideBandPolicy(len_low=20, sustain=2), fake,
        base_knobs={"termination_threshold.foot_pos_xyz": 0.35})
...
def test_config_verify_skipped_for_adapters_without_seam():
    """Plain FakeAdapter has no resolved_config_text: the loop must not
    invent one."""
...
# carry an EXPLICIT motion_file override whenever a held-out hook is wired,
# or eval inherits the checkpoint config's motion set. In core the pin is an
# opaque eval_extra_overrides list; LoopConfig refuses heldout_hook without it.

_EVAL_PIN = ("++manager_env.commands.motion.motion_lib_cfg."
             "motion_file=data/x/robot_curriculum_eval64")
_HELDOUT_PIN = ("++manager_env.commands.motion.motion_lib_cfg."
                "motion_file=data/x/robot_heldout_eval64")


def _heldout_hook(adapter, seg, cum_it, eval_envs):
    """Core-side shape of the old held-out pass (smoke_driver.py:653-662):
    a second eval-only run with its OWN pin, folded to heldout_* keys."""
    raw = adapter.eval_segment(seg, it=cum_it, num_envs=eval_envs,
                               out_suffix="_heldout_eval", raw=True,
                               extra_overrides=[_HELDOUT_PIN])
    return {"it": cum_it,
            "heldout_success_rate": raw.get("eval/success/success_rate"),
            "heldout_failed_count": len(raw.get("failed_keys", []))}


def test_heldout_manifest_without_eval_pin_refused():
    ...
    with pytest.raises(ValueError, match="eval_motion_file"):
        LoopConfig(heldout_hook=_heldout_hook)


def test_standard_eval_pass_pins_motion_file():
    fake = PinCaptureAdapter([(50, 1.0)] * 2, 4)
    driver = make_manager(TrainSideBandPolicy(len_low=20, sustain=2), fake,
                          LoopConfig(heldout_hook=_heldout_hook,
                                     eval_extra_overrides=[_EVAL_PIN]))
    driver.run(2)
...
def test_eval_pin_absent_without_heldout_wiring():
    """No heldout hook, no eval pin: legacy behavior — the adapter is
    called without any motion_file override (checkpoint config rules)."""
...
def test_purge_deletes_intermediates_keeps_snapshots(tmp_path):
    ...
    deleted, freed = simple_purge(str(run))
...
def test_purge_event_journaled_after_verified_segment(tmp_path):
    ...
    driver = make_manager(TrainSideBandPolicy(len_low=20, sustain=2), fake,
                          LoopConfig(purge_fn=simple_purge))
    driver.run(1)
...
def test_purge_noop_without_run_dir():
    """Fakes without experiment_dir: no purge event, no crash — even with
    the hook injected."""
...
def test_failed_segment_not_purged(tmp_path):
    ...
    driver = make_manager(TrainSideBandPolicy(), fake,
                          LoopConfig(purge_fn=simple_purge))
    driver.run(1)
...
def test_disk_gate_blocks_launch_below_8gb(tmp_path, monkeypatch):
    monkeypatch.setattr(loop_mod.shutil, "disk_usage",
                        lambda p: _fake_usage(4 * 1024**3))
    fake = FakeAdapter([(50, 1.0)] * 2)
    driver = make_manager(TrainSideBandPolicy(), fake,
                          LoopConfig(disk_gate_path=str(tmp_path)))
...
def test_disk_gate_off_for_injected_adapters_without_path():
    """The core NEVER self-arms the gate (no BASE_DIR auto-wiring): without
    an explicit path it stays off."""
...
# ladder both v4 manager seeds walked. arm="scripted" becomes the
# scripted_config() flag bundle (gate_on_pending=False,
# retire_open_watch_on_set=True).
...
def run_scripted(script, eval_script, segments=None):
    fake = FakeEvalAdapter(script, eval_script)
    driver = make_manager(ScriptedPolicy(), fake,
                          scripted_config(base_knobs=dict(V4_BASE_KNOBS)))
    summary = driver.run(segments or len(script))
    return driver, fake, summary
...
def test_scripted_choice_ignores_digest_and_eval_state():
    ...
    None) at the same tick yield the identical knob/value."""
...
def test_scripted_control_and_manager_arms_unaffected():
    """Arm selection is additive: existing flag bundles behave exactly as
    before with the ladder module present (spot check)."""
    driver, fake, s = run_driver([(10, 1.0)] * 4, arm="control")
    assert s["decisions_applied"] == 0
    assert all(k == {} for _, k, _ in fake.launched)
